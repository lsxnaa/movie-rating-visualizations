...
### Rating vs. Continuous Attributes

To begin, the following Figure 1 shows the correlation between the movies' continuous attributes and overall rating. Using the slide bar, you can compare the attributes' distributions by different rating level thresholds. By default, the threshold for high ratings is >7.5. This is because the minimum rating among movies in this dataset that occur in the current IMDb Top 1000 chart is 7.6. 

<div style="text-align: left; margin-left: 40px; margin-bottom:0px; top: 15px; position: relative">

<p style="margin-bottom:0px">

Rating Level Threshold: [7.5]{#G1threshold} <br> Low ≤
<input type="range" id="G1slider" name="rating threshold" min="4.5" max="8.5" value="7.5" step="0.5" onchange="updateG1(this.value);" list="steplist">
...
G1longdt$name <- factor(G1longdt$name, levels=c("votes", "runtime", "gross_domestic", "gross_world"), 
                      labels=c("Total Votes (thousands)", "Run Time (mins)", "Domestic Gross ($millions)", "Worldwide Gross ($millions)"))
...
plotG1 <- function(th) {
  ...
  g11 <- ggplot(G1longdt, aes(x=value, y=rating, color=`rating level`)) + 
  ...
  return(subplot(
    ...
    layout(
      ...
      annotations=list(
        list(x=0, y=-0.105, showarrow = F, xref='paper', yref='paper', font=list(size=12), text=paste0("Figure 1. The continuous attributes' correlation with rating and distribution by rating level with threshold ", th, "."))
      )
    ) )
}
...
By the grouped histogram and scatter plot, there is a moderate, positive correlation between the number of votes and the rating. There is also a weak, positive correlation between rating and run time. In comparison, there is not an obvious, significant correlation between rating and the box office variables. 

### Rating vs. Genre and Year

Next, Figure 2 shows the count and mean rating of movies by genre and year. 
...
Firstly, the averaged movie ratings in different years are similar, with slightly higher average ratings in years 2014-2017. 

As for the genres, there are a lot more deviations. There is only one mystery movie and one scientific fiction movie. Although their ratings are above the average, we should not conclude this as a typical phenomenon. Among groups with a reasonably large size, crime, animation and biography movies tend to be highly rated. In addition, a majority of the drama movies have a good rating, with a few exceptionally low ratings in the second half of the decade. On the other hand, horror movies and comedy movies tend to have a lower rating than average. Comedy movies were very popular but received low ratings in the first demi-decade, which is in contrast to the lower counts but higher ratings in the second demi-decade. 

### Rating vs. Star and Director

The treemaps in Figure 3 demonstrate how movie ratings may correlate with star and director frequencies. Each individual tile represents one movie, with its colour determined by the movie rating. These movie tiles are divided into groups. Hovering over a grouping tile yields the count and mean rating of that frequency group. You can also click on an individual movie tile to look at detailed movie information. 
...
plotly_treemap <- function(col="director", p=NA) { 
  ...
  txt <- paste0("No. Movies : ", dt[,.N, by = col_freq][order(get(col_freq), decreasing=T), N], rep("\nMean ", times=length(lab)))
  lab <- c(lab, paste0("Freq : ", dt[,get(col_freq)], '\n"', dt$title, '"'))
...
plotly_treemap("director", p) %>%
  layout(
    ...
    margin=list(l=0, r=0, b=20, t=0),
  annotations=list(list(
    x=0, y=-0.035, xshift=3, showarrow = F, xref='paper', yref='paper',
    font=list(size=12, color="black"), text="Figure 3. Movies and their ratings grouped by star and director frequencies."),
  ...
  images=list(list(source=paste('data:image/png;base64', txt, sep=','), xref='paper', yref='paper', x=0.478, y=0.66, sizex=0.8, sizey=0.27, layer="above"))
  )

```

There are 185 unique stars and 200 unique directors. Based on the size and the hover info of the grouping tiles, over 40% of the movies belong to the *freq 1* group.
In the overall perspective, there is an obvious increase in the mean rating as the director frequency increases. In contrast, we do not observe any significant linear relationship between the star frequency and the movie rating. Surprisingly, the group of frequency 8 is the one with the lowest mean rating. This potentially suggests that the movie quality depends more on the experience of the director rather than the star.
