...
To begin, Figure 1 shows the correlation between rating and continuous attributes through correlation coefficients and scatter plots. In addition, it compares the attributes' histogram distributions between higher-rated and lower-rated movies. Since the mean of the rating distribution is about 7.1, Figure 1 uses >7 (i.e. ≥7.1) as the default definition of high ratings. Using the slide bar, we can set different rating thresholds.

<div style="text-align: left; margin-left: 64px; margin-bottom:0px; top: 20px; position: relative">
...
By the grouped histogram and scatter plot, there is a moderate, positive correlation between the number of votes and the rating. There is also a weak, positive correlation between rating and runtime. In comparison, there is not an obvious, significant correlation between rating and the box office variables. 
...
The treemaps in Figure 3 demonstrate how movie ratings may correlate with star and director frequencies. Each individual tile represents one movie, with its colour determined by its rating. These movie tiles are divided into frequency groups. We can hover over a frequency tile to see the count and mean rating of that frequency group. We can also click on an individual movie tile to look at detailed movie information. Click again to go back.
